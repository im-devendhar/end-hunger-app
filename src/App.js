...
function App() {
  ...
  const foodListClickHandler = async (user) => {
    const userBool = user === 'donor' ? false : true;
    const rawData = { donate_food: userBool };

    try {
      const response = await axios.post('http://127.0.0.1:5000/get_food_list', rawData);
      setJson(response); // Note: consider using response.data if FoodTable expects the payload
    } catch (error) {
      console.error('Food list creation error: ', error);
      alert('Error submitting food donation.');
    }
  ...
  return (
    <Router>
      <Routes>
        ...
        <Route
          path="/food-list"
          element={<FoodTable foodList={json} isRecipient={user === 'donor' ? true : false} />}
        />
      </Routes>
    </Router>
  );
